...
class ParticleFilter:
    ...
    def update_particles_with_odom(self, msg):
        ...
        if self.current_odom_xy_theta:
            ...
            self.current_odom_xy_theta = new_odom_xy_theta
            return
        for particle1 in (self.particle_cloud):
            particle1.x +=delta[0]#x da particula
            particle1.y +=  delta[1]#y da particula
            particle1.theta += delta[2]#angulo da particula
    ...
    def resample_particles(self):
        ...
        self.particle_cloud = ParticleFilter.weighted_values(self.particle_cloud,[p.w for p in self.particle_cloud],len(self.particle_cloud))
        for p in self.particle_cloud:
            p.w = 1./len(self.particle_cloud)

        self.normalize_particles()

# resortear as particulas em uma nova particle cloud com as particulas normalizadas.
#a função de reortar ja esta no codigo  @draw weighted values.
    def update_particles_with_laser(self, msg):
        """ Updates the particle weights in response to the scan contained in the msg """
        l = 0.2 #tamanho do robo
        for i in range(len(msg.ranges)):

            for p in particle_cloud:
                for z in msg.ranges:
                    x= p.x+ l*math.cos(math.radians(i))
                    y=p.y+l*math.sen(math.radians(i))
                    p.w = p.w*norm(mean =msg.ranges[i],std=0.1).pdf(self.occupancy_field.get_closest_obstacle_distance(x,y))
        pass
    ...
    def update_initial_pose(self, msg):
        ...
        for i in range(self.n_particles):
            p = Particle() #cria uma particula
            p.w = 1.0/n_particles #probabilidade da particula
            p.x= xy_theta[0]+random.uniform(1.5,-1.5)#x da particula
            p.y = xy_theta[1]+random.uniform(1.5,-1.5)#y da particula
            p.theta = xy_theta[2]+random.uniform(90,-90)#angulo da particula
            self.particle_cloud.append(p)#adiciona na lista particle cloud direto
